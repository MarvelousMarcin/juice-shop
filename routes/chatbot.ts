  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!token) {
      res.status(200).json({
        ...
        body: `Hi, I can't recognize you. Sign in to talk to ${config.get<string>(
          "application.chatBot.name"
        )}`,
      });
      return;
...
async function getUserFromJwt(token: string): Promise<User | null> {
  return await new Promise((resolve) => {
    ...
    if (decodedHeader?.alg === "none") {
      resolve(null); // Reject tokens with 'none' algorithm
      return;
    }

    // Verify the token with the allowed algorithm
    jwt.verify(
      token,
      security.publicKey,
