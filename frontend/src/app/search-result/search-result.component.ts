...
export class SearchResultComponent implements OnDestroy, AfterViewInit {
  ...
  filterTable() {
    ...
    if (queryParam) {
      ...
      const safeQueryParam = DOMPurify.sanitize(queryParam);

      this.ngZone.runOutsideAngular(() => {
        // vuln-code-snippet hide-start
        this.io.socket().emit("verifyLocalXssChallenge", safeQueryParam);
      }); // vuln-code-snippet hide-end
      this.dataSource.filter = safeQueryParam.toLowerCase();
      this.searchValue = safeQueryParam; // vuln-code-snippet vuln-line localXssChallenge xssBonusChallenge
